...
/**
 * A cmx_font struct represents all data and metadata about a font and it's
...
struct cmx_font {
  struct {
    ...
    struct cmx_font_size size;
    struct cm_color color;
    void * data;
  } metadata;

  struct {
    struct cmx_font_packed_block * packed_blocks;
...
/**
 * Constructs a `cmx_font` value with a font data, a size (either in pixels or
 * points) and a font color in RGBA. It does not have any pixel data yet, you
 * need to call cmx_font_pack to add unicode ranges and produce a 4 bytes per
 * pixel, RGBA, bitmap.
 * @param font A pointer to the `cmx_font` value to construct
 ...
 */
extern void cmx_font_construct(
 ...
 *
 * This function uses data produced by `cmx_font_pack` and information inside
 ...
 * this or else your memory will leak.
 * @param font A pointer to the `cmx_font` value that will be destroyed.
 */
extern void cmx_font_destruct(struct cmx_font * font);
